...
> **上位机**: VOFA+ (FireWater文本协议, 115200bps)
...
Phase 7: 硬件联调                🔄 进行中
  - Phase 7.1: 串口基础通信      ✅ 完成 (VOFA+ 5通道显示正常)
  - Phase 7.2: BH1750光照验证    ⬜ 待进行
  - Phase 7.3: 雨滴传感器验证    ⬜ 待进行
  - Phase 7.4: 步进电机转动验证  ⬜ 待进行
  - Phase 7.5: SG90舵机角度验证  ⬜ 待进行
...
### 上行帧 (STM32→VOFA+)
VOFA+ FireWater文本格式: `"ch0,ch1,ch2,ch3,ch4\n"`
...
**说明**: 原先使用二进制 JustFloat 帧尾格式, 在 VOFA+ 中解析异常。已改为文本 FireWater 格式, 以换行符结尾, VOFA+ 可正确解析 5 通道数据。发送函数也统一改用 `HAL_UART_Transmit`。
...
**环境**: STM32F103C8T6 + CH340 + VOFA+

**关键修改**:
1. 系统时钟从 HSI 64MHz 切回 HSE 8MHz × 9 = 72MHz (真实硬件)
2. TIM2/3/4 Prescaler 恢复为 72MHz 适配值
3. BH1750 从仿真模式 (`SIMULATION_MODE=1`) 改为真实硬件模式 (`SIMULATION_MODE=0`)
4. 启用 `MX_I2C1_Init()` 初始化 I2C1 外设
5. VOFA+ 协议从二进制 JustFloat 改为 **FireWater 文本格式**: `"ch0,ch1,ch2,ch3,ch4\n"`
6. 串口发送从寄存器直接操作改回 **HAL_UART_Transmit**

**验证结果**:
- VOFA+ 成功显示 5 通道数据
- 当前显示: I0=0.00, I1=1.00, I2=0.00, I3=1.00, I4=0.00
- 数据静止原因为传感器未完全接入/状态未改变, 串口链路本身正常

**遗留问题**:
- BH1750 光照值始终为 0, 需检查 I2C 接线/上拉电阻/模块供电
- 雨滴传感器始终为 1(有雨), 可能因未接传感器时 PA0 下拉导致
...
### Phase 7: 硬件联调
- [x] Phase 7.1: 串口基础通信验证 (VOFA+ 5通道数据显示正常)
