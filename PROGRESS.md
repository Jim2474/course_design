...
### 已知副作用 (HSI 64MHz vs HSE 72MHz)
- TIM4 500ms → 约 **562ms**（慢约12.5%）
- TIM3 1ms → 约 **1.125ms**（慢约12.5%）
- 步进电机速度会稍微慢一点
- 舵机 PWM 频率还是 50Hz，不受影响
- 串口波特率可能有微小误差，但仿真环境下通常能正常通信

### 下一步排查计划
1. 排查串口不发送数据的原因（TIM4 中断？VOFA 协议？）
2. 排查舵机不工作的原因（TIM2 PWM？GPIO 配置？）
3. 验证雨滴传感器和按键功能
4. 调整步进电机方向和速度参数
